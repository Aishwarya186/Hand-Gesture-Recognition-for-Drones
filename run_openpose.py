...
try:
   # print("creating vid_list")
    vid_list = [i for i in os.listdir(vid_dir) if isdir(join(vid_dir, i))]
#/bin/bash: q: command not foundi
    #print("created")
    vid_list = np.random.permutation(vid_list)
    #print("done")
except BaseException as e:
    write_log(log, 'Cannot get video lists: {}'.format(e), log_type='FATAL')
#    shutdown()
...
for i in tqdm(vid_list):
    ...
    big_json_name = output_dir + 'big.json'
    ...
    try:
        if isfile(big_json_name):
           ...
           #write_log(log, "exists ", output_dir)
    ...
    if rc.returncode == 0:
        ...
        write_log(log, 'Features extracted for video {}'.format(
            i), log_type='COMPL')
        write_log(log, "trying", os.listdir(output_dir))
    else:
    ...
    try:
        write_log(log, "json dir ", os.listdir(output_dir))
